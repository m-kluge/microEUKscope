...
export CONTIGS_DIR="${CONTIGS_DIR:-/path/to/min500}"     # ${SAMPLE}.contigs_min500.fa
...
TIARA_PARTITION="${TIARA_PARTITION:-core}"
TIARA_CPUS="${TIARA_CPUS:-20}"
TIARA_TIME="${TIARA_TIME:-03:00:00}"
TIARA_ACCOUNT="${TIARA_ACCOUNT:-your-slurm-account}"
...
###############################################################################
# Portable env activation snippet (re-used in both heredocs)
...
__ACTIVATE__
# Export so SLURM jobs inherit it and can eval it at runtime
export ACTIVATE_ENV_SNIPPET="${_ACTIVATE_ENV_SNIPPET}"

###############################################################################
# SUBMIT TIARA ARRAY
###############################################################################
tiara_job_line=$(sbatch -p "$TIARA_PARTITION" --ntasks=1 --cpus-per-task="$TIARA_CPUS" \
  -t "$TIARA_TIME" -A "$TIARA_ACCOUNT" \
  --array=1-$(wc -l < "$SAMPLE_LIST") \
  -J tiara-fungi_arctic --export=ALL <<'EOF_TIARA'
#!/bin/bash -l
set -euo pipefail
...
safe_cat() {
  # Usage: safe_cat OUTFILE FILE [FILE ...]
  local out="$1"; shift
  : > "$out"
  local ok=0 miss=0
  for f in "$@"; do
...
mkdir -p "${SAMPLE_DIR}" "${STATS_DIR}"

SRC_CONTIGS="${CONTIGS_DIR}/${SAMPLE}.contigs_min500.fa"
[[ -s "${SRC_CONTIGS}" ]] || { echo "Missing contigs: ${SRC_CONTIGS}"; exit 1; }

exec > >(tee -a "${SAMPLE_DIR}/${SAMPLE}.tiara.run.log") 2>&1
...
# Rename Tiara class splits from *_contigs_min500.fa -> *_${SAMPLE}.scaffolds.3k.fasta
for cls in eukarya mitochondrion plastid archaea bacteria prokarya unknown; do
  src="${cls}_${SAMPLE}.contigs_min500.fa"
...
# Build the 3k euk pool from the (renamed) euk/mito/plastid
...
tiara_jobid=$(awk '{print $4}' <<<"$tiara_job_line")
echo "Submitted TIARA array: ${tiara_jobid}"

###############################################################################
# SUBMIT KAIJU ARRAY (depends on TIARA finishing OK)
###############################################################################
kaiju_job_line=$(sbatch -p "$KAIJU_PARTITION" --ntasks=1 --cpus-per-task="$KAIJU_CPUS" \
  -t "$KAIJU_TIME" -A "$KAIJU_ACCOUNT" ${KAIJU_CONSTRAINT:+-C "$KAIJU_CONSTRAINT"} \
  --array=1-$(wc -l < "$SAMPLE_LIST") \
  --dependency=afterok:"$tiara_jobid" \
  -J kaiju_contigs --export=ALL <<'EOF_KAIJU'
#!/bin/bash -l
set -euo pipefail
...
exec > >(tee -a "${SAMPLE_DIR}/${SAMPLE}.kaiju.run.log") 2>&1
echo "[`date '+%F %T'`] Start ${SAMPLE} (Kaiju/EukRep)"
...
rm -f "${SAMPLE}.virus_contigs_greedy"
rm -f "${SAMPLE}.prok_contigs_greedy" "${SAMPLE}.prok1_contigs_greedy" "${SAMPLE}.prok2_contigs_greedy"
...
kaiju_jobid=$(awk '{print $4}' <<<"$kaiju_job_line")
echo "Submitted KAIJU array (after TIARA): ${kaiju_jobid}"
